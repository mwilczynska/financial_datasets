...
DATASET = Path("data/processed/long_term_us_treasury.csv")
PARQUET_DATASET = Path("data/processed/long_term_us_treasury.parquet")
RAW_FED = Path("sources/raw/long_term_us_treasury_fed_nominal_yield_curve.csv")
RAW_VUSTX = Path("sources/raw/long_term_us_treasury_yahoo_vustx_chart.json")
RAW_TLT = Path("sources/raw/long_term_us_treasury_yahoo_tlt_chart.json")
MANIFEST = Path("sources/manifests/long_term_us_treasury.yml")
...
def test_long_term_treasury_scaffold_paths_exist():
    assert MANIFEST.exists()
    assert Path("sources/citations/long_term_us_treasury.md").exists()


def test_long_term_treasury_processed_outputs_exist():
...
def test_long_term_treasury_yahoo_compatible_schema():
...
def test_long_term_treasury_coverage_starts_at_1970():
...
def test_long_term_treasury_levels_positive_and_returns_recompute():
    rows = read_csv(DATASET)
    previous_close = None
...
def test_long_term_treasury_segment_returns_match_raw_yahoo_adjusted_close():
    rows = read_csv(DATASET)
    if not RAW_VUSTX.exists() or not RAW_TLT.exists():
        pytest.skip("Raw Yahoo chart payloads are not available")

    vustx_returns = chart_adjusted_returns(RAW_VUSTX)
    tlt_returns = chart_adjusted_returns(RAW_TLT)
    ...
    for row in rows:
        if row["Total Return"] == "":
            continue
        if "VUSTX" in row["Source"] and row["Date"] in vustx_returns:
            assert abs(decimal_value(row["Total Return"]) - vustx_returns[row["Date"]]) <= RETURN_TOLERANCE
            checked_vustx += 1
        if "TLT" in row["Source"] and row["Date"] in tlt_returns:
            assert abs(decimal_value(row["Total Return"]) - tlt_returns[row["Date"]]) <= RETURN_TOLERANCE
            checked_tlt += 1

    assert checked_vustx > 3000
    assert checked_tlt > 5000


def test_long_term_treasury_has_fed_modelled_pre_vustx_segment():
    rows = read_csv(DATASET)
    fed_rows = [
        row
        for row in rows
        if row["Quality Flag"] == "model_fed_yield_curve_25y_par_treasury_total_return"
    ]
    assert RAW_FED.exists()
    assert len(fed_rows) > 4000
    assert fed_rows[0]["Date"] == "1970-01-02"
    assert fed_rows[-1]["Date"] < "1986-05-20"
