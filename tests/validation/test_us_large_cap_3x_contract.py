...
from build_us_large_cap_3x_sp500 import (  # noqa: E402
    SYNTH_FLAG,
    UPRO_FLAG,
    close_series,
    make_irx_lookup,
...
DATASET = Path("data/processed/us_large_cap_3x_sp500.csv")
PARQUET_DATASET = Path("data/processed/us_large_cap_3x_sp500.parquet")
BASE_DATASET = Path("data/processed/us_large_cap_sp500.csv")
IRX_RAW = Path("sources/raw/us_large_cap_3x_sp500_yahoo_irx_chart.json")
UPRO_RAW = Path("sources/raw/us_large_cap_3x_sp500_yahoo_upro_chart.json")
MANIFEST = Path("sources/manifests/us_large_cap_3x_sp500.yml")
CITATION = Path("sources/citations/us_large_cap_3x_sp500.md")
...
def test_segment_flags_present_and_sized():
    ...
    observed = [r for r in rows if r["Quality Flag"] == UPRO_FLAG]
    # Every row is one of the two known flags.
    assert len(synth) + len(observed) == len(rows)
    # Synthetic segment runs from 1970 to UPRO inception (~9,900 trading days).
    assert len(synth) > 9000
    # Observed UPRO segment from 2009 onward.
    assert len(observed) > 3500
    # Synthetic comes first, observed last; the boundary is a single transition.
    flags = [r["Quality Flag"] for r in rows]
    assert flags[0] == SYNTH_FLAG
    assert flags[-1] == UPRO_FLAG
    transitions = sum(1 for a, b in zip(flags, flags[1:]) if a != b)
    assert transitions == 1


# --- Observed UPRO segment matches the raw source --------------------------------


def test_observed_segment_matches_raw_upro_adjusted_returns():
    rows = read_csv(DATASET)
    upro_adj = close_series(load_raw(UPRO_RAW), "adjclose")

    checked = 0
    previous_date: str | None = None
    for row in rows:
        if row["Quality Flag"] == UPRO_FLAG and previous_date is not None:
            cur = upro_adj.get(row["Date"])
            prev = upro_adj.get(previous_date)
...
def test_synthetic_segment_matches_daily_reset_model():
    ...
    # Independently recompute the compounding chain over the synthetic segment.
    level = Decimal("100")
...
# --- Live overlap calibration against UPRO ---------------------------------------


def test_model_tracks_upro_over_live_overlap():
    """Recompute the model over the UPRO live period and compare to actual UPRO."""
    base_rows = read_csv(BASE_DATASET)
    base_tr = {r["Date"]: (Decimal(r["Total Return"]) if r["Total Return"] else None) for r in base_rows}
    base_dates = [r["Date"] for r in base_rows]
    upro_adj = close_series(load_raw(UPRO_RAW), "adjclose")
    get_irx = make_irx_lookup(load_raw(IRX_RAW))
    inception = min(upro_adj)
    ...
    # Daily returns are very highly correlated with actual UPRO.
    assert corr > 0.99
    # Cumulative growth over ~17 years matches within 10% (calibration target).
