"""Build the derived 3x daily-reset intermediate-term U.S. Treasury (TYD-like) total-return dataset.

This dataset models a 3x daily-reset leveraged intermediate-Treasury fund (Direxion Daily 7-10
Year Treasury Bull 3X Shares, ticker TYD) back to 1970, long before TYD's 2009 inception.

Note on naming: the project backlog originally labelled "UST" as the 3x intermediate-Treasury
ETF, but UST (ProShares Ultra 7-10 Year Treasury) is a 2x fund. The genuine 3x 7-10 year
Treasury ETF is Direxion TYD, which is therefore the calibration/validation target here.

Method:
  * Underlying daily total return is taken from the project's ITT dataset
    (``data/processed/intermediate_term_us_treasury.csv``, column ``Total Return``), an IEF-like
    intermediate-Treasury total-return series (Fed yield-curve 8.5y par model -> VFITX -> IEF).
  ...
  * The synthetic segment runs from 1970-01-02 through and including TYD's first trading day;
    from the next trading day onward the dataset uses observed TYD adjusted-close daily total
...
ASSET_ID = "intermediate_term_us_treasury_3x"
ALIAS = "ITT3X"
BASE_ASSET_ID = "intermediate_term_us_treasury"
...
# Model parameters (see methodology_itt3x.md). The borrowing spread is calibrated against the
# TYD live overlap so synthetic cumulative growth matches observed TYD.
LEVERAGE = Decimal("3")
EXPENSE_RATIO = Decimal("0.0109")      # Direxion TYD total annual operating expense ratio (~1.09%).
BORROW_SPREAD = Decimal("0.0019")      # Borrowing spread over ^IRX, annual.
                                       # Calibrated so synthetic cumulative growth matches observed
                                       # TYD over the 2009+ overlap (ratio ~1.0). The low implied
                                       # spread partly reflects TYD fee waivers; daily-return
                                       # correlation is degraded 2014-2018 by stale low-AUM prices.
...
SYNTH_FLAG = "model_3x_daily_reset_synthetic_from_itt_total_return_minus_financing_and_fee"
ETF_FLAG = "observed_tyd_etf_adjusted_total_return"

SYNTH_SOURCE = "Derived 3x daily-reset model (ITT total return; ^IRX+spread financing; Direxion-style fee)"
ETF_SOURCE = "Yahoo Finance chart API (TYD adjusted-close total return)"
SYNTH_NOTES = (
    "Synthetic 3x daily-reset intermediate-Treasury total-return NAV: lev_ret = 3*ITT_total_return "
    "- 2*(^IRX/100)*days/360 - 0.0109*days/365. Close == Adj Close. Model-derived, not observed TYD history."
)
ETF_NOTES = "Observed TYD ETF adjusted-close daily total return. Close == Adj Close (normalized 3x NAV level)."
...
def main() -> None:
    ...
    (raw_dir / f"{ASSET_ID}_yahoo_tyd_chart.json").write_text(json.dumps(etf_payload), encoding="utf-8")
